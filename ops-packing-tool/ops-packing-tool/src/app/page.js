...
const ICONS = { serviceware: '🥄', 'add-ons': '➕', beverages: '🧃', coffee: '☕', alcohol: '🍷' };
const LABELS = { serviceware: 'Serviceware', 'add-ons': 'Add-ons', alcohol: 'Alcohol', beverages: 'Beverages', coffee: 'Coffee' };
const DARK_STYLES = {
...
function DropSlot({ slot, file, state, onFile, onRemove }) {
  ...
  function handleDrop(e) {
    ...
    if (f && f.type === 'application/pdf') onFile(slot, f);
  }

  const shortName = file ? (file.name.length > 20 ? file.name.slice(0, 17) + '…' : file.name) : '';
  ...
  return (
    <div>
      ...
      >
        <input type="file" accept=".pdf" style={{ display: 'none' }} onChange={e => { const f = e.target.files[0]; if (f) onFile(slot, f); e.target.value = ''; }} />
        ...
        {file && state !== 'processing' && state !== 'done' && (
          <div style={styles.fileLoadedInner}>
            ...
            <button type="button" onClick={e => { e.preventDefault(); e.stopPropagation(); onRemove(slot); }} style={styles.fileRemove}>Remove</button>
          </div>
        )}
...
export default function Home() {
  ...
  function removeFile(slot) {
    ...
    setSlotStates(prev => { const n = { ...prev }; delete n[slot]; return n; });
  }
  ...
  function handleReset() {
    setFiles({}); setSlotStates({}); setOrders(null); setError('');
  }
        ...
        <div style="padding:10px 14px;display:flex;flex-direction:column;gap:7px">
          ${cats.map(cat => {
            ...
            return `<div style="display:flex;gap:10px;align-items:flex-start">
              ...
              <div style="display:flex;flex-wrap:wrap;gap:5px">${order.categories[cat].map(i => `<span style="display:inline-flex;align-items:center;gap:5px;background:#f5f5f3;border:1px solid #e5e5e0;border-radius:6px;padding:3px 10px;font-size:12px"><b style="font-family:monospace">${i.qty}</b> <span style="color:#666">${i.name}</span></span>`).join('')}</div>
            </div>`;
          }).join('')}
        </div>
      </div>`;
          ...
          <div style={{ maxWidth: 860, margin: '0 auto', padding: '36px 24px 60px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: '#6a8a9a', marginTop: 4 }}>{today()} · {orders.length} orders · sorted by pickup time</div>
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={handleDownload} style={styles.btnOut}>⬇ Download</button>
                <button onClick={() => window.print()} style={styles.btnOut}>🖨 Print</button>
                <button onClick={handleReset} style={{ ...styles.btnOut, ...styles.btnAccent }}>← New reports</button>
              </div>
            </div>
                ...
                return (
                  <div key={order.order_number} className="order-card" style={{ animationDelay: `${idx * 70}ms` }}>
                    <div style={styles.orderHeader}>
                      ...
                      <span style={{ fontSize: 13, fontWeight: 600, color: order.time ? '#4EA99B' : '#3a5a6a' }}>{order.time || '—'}</span>
                    </div>
    ...
    <>
      ...
      <main style={{ minHeight: '100vh', background: '#08131a', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '36px 24px 60px' }}>
          ...
          <div style={styles.stepLabel}>Step 1 — Upload your reports</div>
          ...
          {loading && (
            <div style={styles.progressRow}>
              ...
              <div style={{ flex: 1, height: 3, background: '#1e3545', borderRadius: 2, overflow: 'hidden' }}>
                <div style={{ height: '100%', background: 'linear-gradient(90deg,#1B6B5A,#4EA99B)', width: progressPct + '%', transition: 'width 0.4s ease', borderRadius: 2 }} />
              </div>
            </div>
          )}
          ...
          >
            {loading ? 'Processing reports…' : `Generate packing sheet${fileCount ? ` — ${fileCount} report${fileCount !== 1 ? 's' : ''} loaded` : ''}`}
          </button>
        </div>
      </main>
    </>
  ...
  return (
    <header style={styles.header}>
      <div>
        ...
        <div style={{ color: 'rgba(255,255,255,0.45)', fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: 2 }}>Operations · Bi-Rite</div>
      </div>
...
const globalStyles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media print {
    ...
    .order-card { background: #fff !important; border-color: #ddd !important; opacity: 1 !important; transform: none !important; animation: none !important; }
  }
`;
...
const styles = {
  ...
  slotHint: { fontSize: 11, color: '#3a5a6a', lineHeight: 1.4 },
  fileLoadedInner: {
    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, width: '100%',
  },
  fileCheck: {
    width: 28, height: 28, borderRadius: '50%', background: '#4EA99B',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    fontSize: 13, color: '#fff', fontWeight: 700, flexShrink: 0,
  },
  fileLoadedName: {
    fontSize: 10, color: '#4EA99B', wordBreak: 'break-all',
    textAlign: 'center', lineHeight: 1.3, fontWeight: 500,
  },
  fileRemove: {
    fontSize: 10, color: '#3a5a6a', background: 'none',
    border: '1px solid #1e3545', borderRadius: 4,
  ...
  progressRow: {
    ...
    background: '#0f1e28', borderRadius: 8, border: '1px solid #1e3545',
  },
  progressDot: {
    width: 8, height: 8, borderRadius: '50%', background: '#4EA99B', flexShrink: 0,
  },
  errorBox: {
    background: '#1a0f0f', border: '1px solid #4a1f1f',
    borderRadius: 8, padding: '12px 16px',
    color: '#f08080', fontSize: 13, marginBottom: 16,
  },
  generateBtn: {
  ...
  generateBtnDisabled: {
    background: '#152434', color: '#3a5a6a', boxShadow: 'none', cursor: 'not-allowed',
  },
  btnOut: {
    padding: '7px 16px', fontSize: 13, fontWeight: 600,
    background: 'none', border: '1px solid #2a4a5a',
    borderRadius: 8, color: '#e8f4f0', cursor: 'pointer',
  },
  btnAccent: { background: '#4EA99B', borderColor: '#4EA99B', color: '#fff' },
  orderHeader: {
    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
    padding: '10px 16px', background: '#152434', borderBottom: '1px solid #1e3545',
  },
  orderNum: {
    fontFamily: 'monospace', fontSize: 11, background: '#08131a',
    color: '#6a8a9a', padding: '2px 8px', borderRadius: 4,
  },
  orderName: { fontSize: 14, fontWeight: 600, color: '#e8f4f0' },
  orderBody: { padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8 },
  catBadge: {
  ...
  itemPill: {
    ...
    background: '#152434', border: '1px solid #1e3545',
    borderRadius: 6, padding: '4px 10px', fontSize: 12,
  },
  itemQty: { fontWeight: 700, color: '#4EA99B', fontSize: 12 },
  itemName: { color: '#6a8a9a' },
};
